...
INFO:
    ...
    - REF:  https://github.com/mahmoudnafifi/HistoGAN
...
class RGBHistogram(nn.Module):
    """
    INFO
    REF: https://github.com/mahmoudnafifi/HistoGAN
    """
    def __init__(self, h=64, insz=150, resizing='interpolation', method='inverse-quadratic', sigma=0.02, intensity_scale=True, device='cpu'):
        super().__init__()
        ...
        self.h = h
        self.insz = insz #  maximum size of the input image
        self.device = device
        self.resizing = resizing # resizing method if applicable: 'interpolation' or 'sampling'
        self.method = method # the method used to count the number of pixels for each bin in the  histogram feature. Options are: 'thresholding', 'RBF' (radial basis function), or 'inverse-quadratic'
        self.intensity_scale = intensity_scale #  if the method value is 'RBF' or 'inverse-quadratic', then this is  the sigma parameter of the kernel function.
    ...
    def compute_histogram(self, x):
        ...
        if x.shape[2] > self.insz or x.shape[3] > self.insz:
            if self.resizing == 'interpolation':
                x_sampled = F.interpolate(x, size=(self.insz, self.insz), mode='bilinear', align_corners=False)
            
            elif self.resizing == 'sampling':
                inds_1 = torch.LongTensor( np.linspace(0, x.shape[2], self.h, endpoint=False)).to(device=self.device)
                inds_2 = torch.LongTensor( np.linspace(0, x.shape[3], self.h, endpoint=False)).to(device=self.device)
                
                x_sampled = x.index_select(2, inds_1)
                x_sampled = x_sampled.index_select(3, inds_2)
                
            else:
                raise Exception(f'Wrong resizing method. It should be: interpolation or sampling. '
        ...
        hists = torch.zeros((x_sampled.shape[0], 3, self.h, self.h)).to(device=self.device)
        ...
        for l in range(L):
            ...
            if self.method == 'thresholding':
                ...
                raise Exception(f'Wrong kernel method. It should be either thresholding, RBF,' 
        ...
        divide_x =  ((hists.sum(dim=1)).sum(dim=1)).sum(dim=1).view(-1, 1, 1, 1)
        

        hists_normalized = hists / (divide_x + EPS)

        return hists_normalized
    def forward(self, src, tar):
        self.target_hist = self.compute_histogram(tar)
        self.input_hist = self.compute_histogram(src)
        histogram_loss = (1/np.sqrt(2.0) * (torch.sqrt(torch.sum( torch.pow(torch.sqrt(self.target_hist) - torch.sqrt(self.input_hist)+1e-10, 2)))) 
