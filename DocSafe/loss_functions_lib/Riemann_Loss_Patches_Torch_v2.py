...
class Riemann_Patcher_Loss(nn.Module):
    def __init__(self, 
                batch_size:int=1, 
                image_size:int=16,
                # embed_dim:int=16, 
                ...
                in_channels:int=3,
                device=""):
        super(Riemann_Patcher_Loss, self).__init__()
    ...
    def forward(self, 
        ...
        original_image = F.interpolate(original_image, 
        ...
        # img = img.view(self.batch_size*patch_num, 3, (self.patch_size ** 2))
        # generated = generated.view(self.batch_size*patch_num, 3, (self.patch_size ** 2))

        # generated = self.shape_generated_layer(generated)

        
        ## compute distance in riemann space
        loss = self.distance_riemann((img, generated))
        

        ## Normalize the loss value
        loss_op = loss/((self.batch_size)*1.0)

        ## (????)
        # loss_c = torch.nn.functional.binary_cross_entropy_with_logits(
        #                                     tf.ones_like(fake_output_disc), 
        #                                     fake_output_disc)
        out = torch.mean(loss_op) #/5000 + loss_c ## (????)
        del(loss_op)
        del(loss)
        del(img)
        del(generated)
