...
# from ..PSTNet.Probabilistic_Special_Transform_Network_v01 import PSTN, CelebaPSTN
...
class MessagePreparationNetworks(nn.Module):
    def __init__(self, batch_size, 
        ...
        self.dense_2 = nn.Linear(input_shape[0]*input_shape[1]*3, 
                                 128*128*3).to(device)  # Equivalent to Dense(64)
        ...
        # ## Activation
        self.act_2 = Snake(beta=0.5, trainable=True).to(device) # torch.nn.GELU() || Snake(beta=0.5, trainable=True)
        
        self.reshape_2 = lambda x: x.view(self.batch_size, 3, 128, 128).to(device)  
    ...
    def forward(self, inputs, mask):
        ...
        wavelet_x  = self.wavelet_message_layer2(dev_x)
