...
class AttentionVnetDecoder(nn.Module):
    def __init__(self, 
        ...
        #                             padding      = 0, 
        #                             bias         = True)
        self.stn_router = STN(filters=32, batch=batch_size, channel=32)
        #########################################################
        ####                                       ####
        #########################################################
        ...
        self.down_stack = nn.ModuleList([
            EncoderBlock(32, 64, 4, apply_batchnorm=False),  # (bs, 128, 128, 64)
        ...
        ####                                       ####
        #########################################################
        self.last_layer = nn.Conv2d(in_channels  = 64, 
                                    out_channels = 3, 
                                    kernel_size  = 1, 
                                    stride       = 1,
                                    padding      = 0, 
    ...
    def forward(self, inputs, mask):
        ...
        wavelet_encimg = self.wavelet_encoded_layer(dev_encimg)
        ...
        ####                                       ####
        #########################################################
        x = x1 = self.stn_router(xencoded)
        #########################################################
        ####                                       ####
        #########################################################
        # if self.using_affine:
        #     x, boxes, cropped_mask = self.affine_router(x, mask)
        #     croped = x
