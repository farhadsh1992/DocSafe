...
class MessagePreparationNetworks(nn.Module):
    def __init__(self, batch_size, 
        ...
        # self.conv1d = nn.Conv1d(1, 1024, kernel_size=1, stride=1, padding=0).to(device)
        ...
        self.reshape_1 = lambda x: x.view(self.batch_size, 3, 128, 128).to(device)  
        self.padding_1 = torch.nn.ZeroPad2d(64)
        # self.reshape_1 = lambda x: x.view(self.batch_size, 32, 64, 64).to(device)  

        # self.last_layer = SeparableConv2d(in_channels=3, out_channels=256, kernel_size=1, 
    ...
    def forward(self, inputs, mask):
        ...
        out = self.reshape_1(x)  # Reshape
        out = self.padding_1(out)
        mask64 = self.mask_layer(mask)
        out = torch.multiply(out, mask64)
