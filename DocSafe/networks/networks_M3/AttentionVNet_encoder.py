...
########
from .message_preparation_networks_v07 import MessagePreparationNetworks
# from .message_preparation_networks_v11 import MessagePreparationNetworks
# from .message_preparation_networks_v12 import MessagePreparationNetworks
########################################
# from ..Harmonic_MPN.message_preparation_networks_v07 import MessagePreparationNetworks
...
class AttentionVnetEncoder(nn.Module):
    def __init__(self, 
        ...
        ####                                                          ####
        #########################################################################
        self.down_stack = nn.ModuleList([ ## 160 288
            EncoderBlock(288+3, 128, 4).to(self.device),  # (bs, 64, 64, 128)
        ...
        ####                      160 224 352                               ####
        #########################################################################
        self.conv9  = nn.Conv2d(352+3, 64, kernel_size=3, padding=1)
        self.conv10 = nn.Conv2d(64, 32, kernel_size=3, padding=1)
        self.last_layer = nn.Conv2d(32, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, images, mask,  secrets):
        ...
        wavelet_image = self.wavelet_image_layer(inputs=images, sobel= dev_image)
        ...
        embedding       = self.embedding_layer(secrets, images)
        ...
        ####                                       ####
        #########################################################
        x = x1 = torch.cat([embedding, ximage, mask], dim=1)
        # print(tcolors.RED,
        #       "x: ",
        #       x.shape, tcolors.ENDC)
        ...
        return residual, embedding#, effine_en, boxes
...
class Conv1DLayer1_en(nn.Module):
    def __init__(self, 
                filiters = 2,
                batch_size=5, 
                channel=64, 
                input_size=256, 
                device=""):
        super(Conv1DLayer1_en, self).__init__()
        #######################################
        ###                         ###
        #######################################
        self.batch_size  = batch_size
        ...
        self.activation = Snake(beta=0.5, trainable=True).to(device)
        self.reshape    = nn.Unflatten(1, (channel, input_size, input_size))
        self.reshape_last = lambda x: x.view(self.batch_size, 
                                             self.channel*filiters, 
