...
def random_blur_kernel(probs, N_blur, sigrange_gauss, sigrange_line, wmin_line):
    N = N_blur
    coords = torch.from_numpy(np.stack(np.meshgrid(range(N_blur), 
                            range(N_blur), indexing='ij'), axis=-1)) - (0.5 * (N-1)) # （7,7,2)
...
class rgb_to_ycbcr_jpeg(nn.Module):
    ...
    def __init__(self, device):
        ...
        self.device = device
        matrix = torch.from_numpy(np.array(
            [[0.299, 0.587, 0.114], [-0.168736, -0.331264, 0.5],
             [0.5, -0.418688, -0.081312]], dtype=np.float32).T).to(self.device)
        self.shift = nn.Parameter(torch.tensor([0., 128., 128.])).to(self.device)
        self.matrix = nn.Parameter(matrix).to(self.device)
...
class compress_jpeg(nn.Module):
    ...
    """
    def __init__(self, rounding=torch.round, factor=1, device=None):
        super(compress_jpeg, self).__init__()

        self.device = device 
        
        self.l1 = nn.Sequential(
            rgb_to_ycbcr_jpeg(device).to(self.device),
            chroma_subsampling().to(self.device)
        ).to(self.device)
        ...
        self.c_quantize = c_quantize(rounding=rounding, factor=factor).to(self.device)
        self.y_quantize = y_quantize(rounding=rounding, factor=factor).to(self.device)
...
class decompress_jpeg(nn.Module):
    ...
    Ouput:
        image(tensor): batch x 3 x height x width
    """
    def __init__(self, height, width, rounding=torch.round, factor=1, device=None):
        ...
        self.idct = idct_8x8().to(self.device)
        self.merging = block_merging().to(self.device)
        self.chroma = chroma_upsampling().to(self.device)
        self.colors = ycbcr_to_rgb_jpeg().to(self.device)
        
        self.height, self.width = height, width
        
    def forward(self, y, cb, cr, device):
        components = {'y': y, 'cb': cb, 'cr': cr}
        for k in components.keys():
            if k in ('cb', 'cr'):
...
def jpeg_compress_decompress(image,
    ...
    compress = compress_jpeg(rounding=rounding, factor=factor, device=device)
    decompress = decompress_jpeg(height, width, rounding=rounding, factor=factor, device=device)

    y, cb, cr = compress(image, device=device)
    recovered = decompress(y, cb, cr, device=device)
